...
{
    /// <summary>
    /// cash bonus controller
    /// </summary>
    public class CashBonusController : ApiController
    {
        ...
        public CashBonusController(ICashBonusManager cashManager)
        {
            _cashBonusManager = cashManager;
        }
        ...
        public HttpResponseMessage CalculateCashBonus(string groupID, float saleLitre)
        {
            ...
            var cashBonus = _cashBonusManager.CalculateCashBonus(groupID,saleLitre,out error);

            if (!string.IsNullOrEmpty(error.MessageStyle.Message))
            {
                _performancelog.Debug(
                    $"End,CashBonusController,CalculateCashBonuss,{DateTime.Now.Subtract(dateStart).TotalMilliseconds},{DateTime.Now:hh.mm.ss.ffffff}");
            ...
            return Request.CreateResponse(HttpStatusCode.OK,cashBonus);
        }
